...
public class ImageController {
    ...
    //The logic is to get the image from the database with corresponding title. After getting the image from the database the
    // details are shown
    //First receive the dynamic parameter in the incoming request URL in a string variable 'title'  or in the
    // Integer variable 'imageId 'and also the Model type object. Call the getImage() method in the business logic to
    //fetch all the details of that image. getImage() method is used because if there are 2 images with the same title, both
    //images will be displayed. If we use imageByTitle() method, error will be thrown since the title of the 2 images are
    //same. Add the image in the Model type object with 'image' as the key. Return 'images/image.html' file
    ...
    //The method receives all the details of the image to be stored in the database, and now the image will
    // be sent to the business logic to be persisted in the database
    ...
    public String editImage(@RequestParam("imageId") Integer imageId, Model model, HttpSession session) {
        ...
        Comment comment = commentService.getCommentByImage(imageId);

        //Retrieving the current user logged into the Image Hoster application
        User user = (User) session.getAttribute("loggeduser");
        //checking whether the logged in user is the owner of the image
        boolean validUser = (image.getUser().getId()== user.getId());

        //An 'if' condition to edit if the logged in user is the owner of the image.
        //If the logged in user is not the owner then the message in the 'else' condition will be displayed
        if(validUser){
            model.addAttribute("image", image);
    ...
    //Try to get the tag from the database using getTagByName() method. If tag is returned, you need not to store that
    //tag in the database, and if null is returned, you need to first store that tag in the database and then the tag
    //is added to a list. After adding all tags to a list, the list is returned
    ...
    public String addComment(@PathVariable("imageId") Integer imageId, @PathVariable("title") String title,
                             @RequestParam("comments") String comment, Comment newComment, HttpSession session){
